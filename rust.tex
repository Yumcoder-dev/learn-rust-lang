...
\begin{frame}[fragile]
	\frametitle{Pattern Syntax, Matching Ranges of Values with \mintinline{rust}|..=|}
...
\begin{frame}[fragile]
	...
	To switch to unsafe Rust, use the \mintinline{rust}|unsafe| keyword and then start a new block that holds the unsafe code.\textbf{ You can take five actions in unsafe Rust that you can’t in safe Rust}, which we call unsafe superpowers. Those superpowers include the ability to:
...
\begin{frame}[fragile]
	...
	Compiler ensures \textbf{references are always valid}. Unsafe Rust has two new types called \textbf{raw pointers that are similar to references}. As with references, r\textbf{aw pointers can be immutable or mutable} and are written as \mintinline{rust}|*const T| and \mintinline{rust}|*mut T| , respectively. The \textbf{asterisk} isn’t the dereference operator; \textbf{it’s part of the type name}. In the context of raw pointers, immutable means that the pointer can’t be directly assigned to after being dereferenced.
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item 	 When we use the \mintinline{rust}|next|  method on Counter, we would have to provide type annotations to indicate which implementation of Iterator we want to use.
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item 	Sometimes referred to as \textbf{DST}s or \textbf{unsized types}, these types let us write code using values whose size we can know only at runtime.
		 \begin{itemize}
		 	\item That’s right, not \mintinline{rust}|&str|, but \mintinline{rust}|str| on its own, is a DST. We can’t know how long the string is until runtime, meaning we can’t create a variable of type \mintinline{rust}|str|, nor can we take an argument of type \mintinline{rust}|str|.
		 	\item 	 We can combine \mintinline{rust}|str| with all kinds of pointers: for example, \mintinline{rust}|Box<str>| or \mintinline{rust}|Rc<str>|. In fact, you’ve seen this before but with a different dynamically sized type: traits. Every trait is a dynamically sized type.
		 \end{itemize}
	\end{itemize}
...
\begin{frame}[fragile]
	...
	\scriptsize
	\begin{itemize}
		\item A trait bound on  \mintinline{rust}|?Sized| means “\textbf{T may or may not be Sized}” and this notation \textbf{overrides the default that generic types must have a known size at compile time}. \textbf{The \mintinline{rust}|?Trait| syntax with this meaning is only available for Sized, not any other traits}.
		\item 	Also note that we switched the type of the t parameter from \mintinline{rust}|T| to \mintinline{rust}|&T|. Because the type might not be Sized, we need to use it behind some kind of pointer. In this case, we’ve chosen a reference.
	\end{itemize}
...
\begin{frame}[fragile]
	...
	\inputminted{rust}{./code/fn2.rs}
\end{frame}


\begin{frame}[fragile]
	\frametitle{Macros(like \mintinline{rust}|println!|)}
	\begin{itemize}
		\item Fundamentally, macros are \textbf{a way of writing code that writes other code}, which is known as \textbf{metaprogramming}. 
		\item Macros can take a variable number of parameters: we can call \mintinline{rust}|println!("hello")| with one argument or \mintinline{rust}|println!("hello {}", name)| with two arguments. 
		\item The term macro refers to a family of features in Rust: 
		\begin{itemize}
			\item \textbf{Declarative macros} with \mintinline{rust}|macro_rules!|  and 
			\item 	Three kinds of \textbf{procedural macros}:
			\begin{itemize}
				\item \textbf{Custom \mintinline{rust}|#[derive]| macros} that specify code added with the derive attribute used on \mintinline{rust}|structs| and \mintinline{rust}|enums|
				\item 	\textbf{Attribute-like} macros that define custom attributes usable on any item
				\item 	\textbf{Function-like macros} that look like function calls but operate on the tokens specified as their argument
			\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	\frametitle{Declarative Macros with \mintinline{rust}|macro_rules!| for General Metaprogramming}

The most widely used form of macros in Rust is the declarative macro. These are also sometimes referred to as “\textbf{macros by example},” “\mintinline{rust}|macro_rules!| macros,” or just plain “\textbf{macros}.” At their core, declarative macros allow you to write something similar to a Rust \textbf{match expression}.

	\inputminted{shell}{./code/macro2.rs}
...
\begin{frame}[fragile]
	\frametitle{Declarative Macros Example}
	\inputminted[fontsize=\scriptsize]{shell}{./code/macro1.rs}
\end{frame}

\begin{frame}[fragile]
	\frametitle{Declarative Macros Example(2)}
	First, we use a set of parentheses to encompass the whole pattern. We use a dollar sign \mintinline{shell}|($)| to declare a variable in the macro system that will contain the Rust code matching the pattern. The dollar sign makes it clear this is a macro variable as opposed to a regular Rust variable. Next comes a set of parentheses that captures values that match the pattern within the parentheses for use in the replacement code. Within \mintinline{shell}|$()| is \mintinline{shell}|$x:expr|, which matches any Rust expression and gives the expression the name \mintinline{shell}|$x|.
	The comma following \mintinline{shell}|$()| indicates that a literal comma separator character could optionally appear after the code that matches the code in \mintinline{shell}|$()|. The \mintinline{rust}|*| specifies that the pattern matches zero or more of whatever precedes the \mintinline{rust}|*|.
	When we call this macro with \mintinline{rust}|my_vec![1, 2, 3];|, the \mintinline{shell}|$x| pattern matches three times with the three expressions 1, 2, and 3.
\end{frame}

\begin{frame}[fragile]
	\frametitle{Declarative Macros Example(3)}
\inputminted{rust}{./code/macro3.rs}

For more examples see:  \href{https://veykril.github.io/tlborm/decl-macros/macros-methodical.html}{“\textbf{\underline{The Little Book of Rust Macros}}”}

\end{frame}


\begin{frame}[fragile]
	\frametitle{How to Write a Custom derive Macro}
\begin{itemize}
	\item 	\textit{\textbf{Procedural macro}}, which acts more like a function (and is a type of procedure). Procedural macros accept some code as an input, operate on that code, and produce some code as an output rather than matching against patterns and replacing the code with other code as declarative macros do.
	\item Let’s create a crate named \mintinline{rust}|hello_macro| that defines a trait named \mintinline{rust}|HelloMacro| with one associated function named \mintinline{rust}|hello_macro|. Rather than making our users implement the \mintinline{rust}|HelloMacro| trait for each of their types, we’ll provide a procedural macro so users can annotate their type with \mintinline{rust}|#[derive(HelloMacro)]| to get a default implementation of the \mintinline{rust}|hello_macro function|.
	\inputminted[fontsize=\scriptsize]{rust}{./code/macro4.rs}
\end{itemize}
\end{frame}


\begin{frame}[fragile]
	\frametitle{How to Write a Custom derive Macro(2)}
		\inputminted[fontsize=\scriptsize]{rust}{./code/macro5.rs}
\end{frame}

\begin{frame}[fragile]
	\frametitle{How to Write a Custom derive Macro(2)}
	\begin{itemize}
		\item We need functionality from the syn and quote crates.
		\item 	The \mintinline{rust}|syn| crate parses Rust code from a string into a data structure that we can perform operations on. 
		\item 	The \mintinline{rust}|quote| crate turns syn data structures back into Rust code.
		\item The \mintinline{rust}|hello_macro_derive| function first converts the input from a \mintinline{rust}|TokenStream| to a data structure that we can then interpret and perform operations on. This is where syn comes into play. The parse function in syn takes a \mintinline{rust}|TokenStream| and returns a \mintinline{rust}|DeriveInput struct| representing the parsed Rust code.
	\end{itemize}
...
\begin{frame}[fragile]
	...
	\inputminted{rust}{./code/macro6.rs}
\end{frame}



\begin{frame}[fragile]
	\frametitle{Attribute-like macros}
	
	\inputminted{rust}{./code/macro7.rs}
	
	Here, we have two parameters of type \mintinline{rust}|TokenStream|. The first is for the contents of the attribute: the \mintinline{rust}|GET|, \mintinline{rust}|"/"| part. The second is the body of the item the attribute is attached to: in this case, \mintinline{rust}|fn index() {}| and the rest of the function’s body.
...
\begin{frame}[fragile]
	...
	\inputminted{rust}{./code/http15.rs}
\end{frame}


\begin{frame}[fragile]
	\frametitle{Graceful Shutdown and Cleanup, Implementing the Drop Trait on ThreadPool(3)}
	\inputminted{rust}{./code/http16.rs}
\end{frame}

\begin{frame}[fragile]
	\frametitle{Signaling to the Threads to Stop Listening for Jobs}
	\begin{itemize}
		\item The key is the logic in the closures run by the threads of the Worker instances: at the moment, we call join, but that \textbf{won’t shut down the threads} because they \textbf{loop forever} looking for jobs.
		\item 	\textbf{To fix this problem}, we’ll need a \textbf{change} in \textbf{the ThreadPool drop implementation} and then a \textbf{change in the Worker loop}.
	\end{itemize}
\end{frame}

\begin{frame}[fragile]
	\frametitle{Change in the ThreadPool drop implementation}
	\inputminted[fontsize=\fontsize{8}{8}]{rust}{./code/http17.rs}
\end{frame}

\begin{frame}[fragile]
	\frametitle{Change in the ThreadPool drop implementation(2)}
	\inputminted{rust}{./code/http18.rs}
\end{frame}


\begin{frame}[fragile]
	\frametitle{Change in the Worker loop}
	\inputminted{rust}{./code/http19.rs}
\end{frame}
